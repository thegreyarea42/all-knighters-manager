...
void main() {
  ...
  group('Round 1 — Parity mode (default)', () {
    test(
      'even count: pairs players by ascending handicap (strongest vs 2nd-strongest, etc.)',
      () {
        final players = [
        ...
        final result = PairingEngine.generatePairings(players, isRound1: true);

        expect(result.length, 2);
        expect(result.every((pair) => !pair.isBye), isTrue);
        expect(
          _matchExists(result, 'p1', 'p2'),
          isTrue,
          reason: 'Strongest two should be paired',
        );
        expect(
          _matchExists(result, 'p3', 'p4'),
          isTrue,
    ...
    test(
      'odd count: BYE goes to the WEAKEST player (highest numeric handicap value)',
      () {
        final players = [
  ...
  group('Round 1 — Random mode', () {
    test(
      'even count: every player appears exactly once across both colors',
      () {
        final players = [
  ...
  group('Round 1 — Seeded mode', () {
    test('pairs i with i + half (Swiss seeding, not fold pairing)', () {
      ...
      expect(result.length, 2);
      expect(
        _matchExists(result, 'p1', 'p3'),
        isTrue,
        reason: 'index 0 paired with index 2 (i+half)',
      );
      expect(
        _matchExists(result, 'p2', 'p4'),
        isTrue,
  ...
  group('Round 2+ — Swiss', () {
    test(
      'sorts by earnedPoints descending; highest earner faces next highest non-repeating opponent',
      () {
        final players = [
    ...
    test('no-repeat opponent rule: skips prior matchups from previous round', () {
      ...
      expect(result.every((pair) => !pair.isBye), isTrue);
      expect(
        _matchExists(result, '1', '3'),
        isFalse,
        reason: 'No-repeat rule: 1 should NOT face 3 again',
      );
      expect(
        _matchExists(result, '2', '4'),
        isFalse,
        reason: 'No-repeat rule: 2 should NOT face 4 again',
      );
      // 1 must be paired against the only remaining valid opponent: 2
      expect(
        _matchExists(result, '1', '2'),
        isTrue,
        reason:
            'Greedy Swiss pairs 1 (highest) with the first non-repeating opponent on the list',
      );
    });

    test(
      'returns no Swiss pairings when every possible pair has already played',
      () {
    ...
    test('odd count: BYE goes to lowest-point player without prior BYE', () {
      ...
      expect(byes.length, 1);
      expect(
        byes.first.whitePlayerId,
        'low',
        reason: 'Lowest-point player with hadBye=false should receive the BYE',
      );
    });

    test(
      'odd count: falls back to the lowest-point player when everyone has had a BYE',
      () {
        final players = [
    ...
    test(
      'color balance: higher colorBalance player gets BLACK in next pairing',
      () {
        ...
        expect(
          result.first.whitePlayerId,
          'b',
          reason: 'B (more blacks so far) is assigned WHITE',
        );
        expect(
          result.first.blackPlayerId,
          'a',
    ...
    test('color balance: lower colorBalance player gets WHITE', () {
      ...
      final players = [
        ...
        _p('b', 'B', earnedPoints: 1.0, colorHistory: const [ChessColor.white]),
      ];
    ...
    test(
      'color balance: equal balance falls back to p1.last-color alternation',
      () {
